...
def _validated_value(maybe_value: int | None, default_value: int = 0) -> int:
...
class Instruction:
    ...
    def __init__(self, cmd: int, data_value: int | None = None):
    ...
    def frame(self) -> list[int]:
        return [0xAA, 0xBB, 0x03, self.id, self.data_value, 0xEE]

    def __str__(self) -> str:
        return f"<Instruction id: {self.id} name: {self.name} data_value: {self.data_value}>"

    def __repr__(self) -> str:
        return f"Instruction<{self.name}>({self.id}, {self.data_value})"
...
class TcpDevice:
    ...
    def process(
        self, instructions: list[Instruction] | Instruction
    ) -> list[Instruction]:
        try:
            _ = iter(instructions)
        except TypeError:
    ...
    def _create_connection(self) -> socket.socket:
        conn = socket.create_connection((self._endpoint.host, self._endpoint.port))
        conn.settimeout(self._endpoint.timeout_sec)
        return conn

    def _execute_instruction(
        self, instruction: Instruction, conn: socket.socket
    ) -> list[Instruction]:
        ...
        try:
            while bytes_received < Instruction.SIZE_BYTES:
                chunk = conn.recv(
                    min(Instruction.SIZE_BYTES - bytes_received, Instruction.SIZE_BYTES)
                )
                if chunk == b"":
